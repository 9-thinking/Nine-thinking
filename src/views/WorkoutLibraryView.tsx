  ...
  return (
    <div className="space-y-gutter pb-32 font-inter">
      ...
      <header className="px-margin-mobile md:px-0 flex justify-between items-end">
        <div>
          <h1 className="font-outfit font-black text-4xl md:text-5xl text-primary tracking-tighter mb-2">Fitness Center</h1>
          <p className="text-on-surface-variant font-medium">Professional routines tailored to your lifestyle.</p>
        </div>
        <button 
      ...
      <section className="px-margin-mobile md:px-0">
        <div className="grid grid-cols-12 gap-6">
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="font-outfit font-black text-xl text-on-surface tracking-tight">Daily Fitness Goal</h3>
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <span className="text-sm font-bold text-on-surface-variant">Active Calories</span>
                    <span className="text-lg font-black text-on-surface">450 / 600 kcal</span>
                  </div>
              ...
              <div className="flex flex-row md:flex-col gap-6 shrink-0">
                <div>
                  <p className="text-[10px] font-black text-on-surface-variant/40 uppercase tracking-widest mb-1">Consistency</p>
                  <p className="text-2xl font-outfit font-black text-primary">85%</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-on-surface-variant/40 uppercase tracking-widest mb-1">Streak</p>
                  <p className="text-2xl font-outfit font-black text-secondary">5 Days</p>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div>
              <h3 className="font-outfit font-black text-2xl mb-1">Quick Start</h3>
              <p className="text-white/60 text-sm">Resume your custom 15-min cardio blast.</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="flex gap-4 overflow-x-auto pb-4 -mx-margin-mobile px-margin-mobile md:mx-0 md:px-0 no-scrollbar">
        {categories.map((cat, i) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${activeTab === cat.name ? 'bg-primary text-white' : 'bg-surface-container-high group-hover:bg-primary/10'}`}>
              <cat.icon className={`w-5 h-5 ${activeTab === cat.name ? 'text-white' : 'text-on-surface-variant group-hover:text-primary'}`} />
            </div>
            <div>
              <p className="font-outfit font-bold text-lg text-on-surface leading-none mb-1">{cat.name}</p>
              <p className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant/40">{cat.tag}</p>
            </div>
          </button>
        ))}
      </section>
      ...
      <section className="px-margin-mobile md:px-0">
        <motion.div 
          className="bento-card overflow-hidden bg-on-surface text-white border-none min-h-[350px] flex flex-col md:flex-row relative group cursor-pointer"
        >
          <div className="flex-1 p-8 md:p-12 z-10 flex flex-col justify-center">
            ...
            <h2 className="font-outfit font-black text-4xl md:text-6xl mb-6 tracking-tighter leading-[1.1]">Elite Power <br/>Building</h2>
      ...
      <section className="px-margin-mobile md:px-0">
        <div className="flex justify-between items-end mb-8">
          <h3 className="font-outfit font-black text-2xl text-on-surface tracking-tight">Suggested for You</h3>
          ...
          {workouts.map((workout, idx) => (
            ...
            >
              <div className="aspect-[4/3] relative overflow-hidden">
                ...
                <div 
                  onClick={(e) => { e.stopPropagation(); handleStartWorkout(workout.title); }}
              ...
              <div className="p-6 bg-white">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleAddToPlan(workout.title); }}
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex items-center gap-1.5 text-on-surface-variant/60">
                    ...
                    <span className="text-xs font-bold">{workout.calories}</span>
                  </div>
                </div>
              </div>
            </motion.div>
