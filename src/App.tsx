...
export default function App() {
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'login': return <LoginView onNavigate={setView} onLogin={handleLogin} />;
      case 'onboarding': return <OnboardingView onNavigate={setView} user={user} onUpdate={handleLogin} />;
      case 'dashboard': return <DashboardView onNavigate={setView} user={user} />;
      case 'vision-log': return <VisionLogView onNavigate={setView} user={user} />;
      case 'workouts': return <WorkoutLibraryView onNavigate={setView} user={user} />;
      case 'progress': return <ProgressView onNavigate={setView} user={user} />;
      case 'profile': return <ProfileView onNavigate={setView} user={user} onLogout={handleLogout} />;
      case 'register': return <RegisterView onNavigate={setView} onLogin={handleLogin} />;
      case 'forgot-password': return <ForgotPasswordView onNavigate={setView} />;
      case 'providers': return <ProvidersView onNavigate={setView} onSelectDoctor={(doc) => { setSelectedDoctor(doc); setView('chat'); }} />;
      case 'chat': return <ChatView onNavigate={setView} doctor={selectedDoctor} />;
      case 'notifications': return <NotificationsView onNavigate={setView} />;
      default: return <LandingView onNavigate={setView} />;
    }
  };

  const showNavbar = !['landing', 'login', 'onboarding', 'register', 'forgot-password', 'chat', 'notifications'].includes(view);
        ...
        <header className="glass-nav border-b border-outline-variant/5 px-margin-mobile md:px-margin-desktop py-5 flex justify-between items-center transition-all duration-300">
          ...
          >
            <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center group-hover:rotate-6 transition-transform">
              <span className="text-white font-outfit font-black text-lg">N</span>
            </div>
            <span className="font-outfit font-black text-2xl text-primary tracking-tighter group-active:scale-95 transition-transform">Nine Thinking</span>
          </div>
          ...
          <nav className="hidden md:flex items-center bg-surface-container-low/50 p-1.5 rounded-full border border-outline-variant/10">
            {[
              ...
              >
                ...
                <navItem.icon className={`relative z-10 w-4 h-4 ${view === navItem.id ? 'text-primary' : 'text-on-surface-variant/70'}`} />
          ...
          <div className="flex items-center gap-5">
            ...
            >
              <div className={`w-11 h-11 rounded-full border-2 p-0.5 cursor-pointer transition-all active:scale-95 duration-300 ${view === 'profile' ? 'border-primary ring-4 ring-primary/5' : 'border-outline-variant/20 hover:border-primary/50'}`}>
                <img 
      ...
      {showNavbar && (
        <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] z-50 bg-white/90 backdrop-blur-2xl border border-outline-variant/10 px-2 py-2 flex justify-around items-center rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.1)]">
          {[
            ...
            { id: 'vision-log' as ViewState, label: 'Scan', icon: Camera, special: true },
            { id: 'progress' as ViewState, label: 'Trends', icon: TrendingUp },
            ...
            if (navItem.special) {
              return (
                ...
                >
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest mt-1 text-primary">{navItem.label}</span>
                </button>
              );
            }
            return (
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">{navItem.label}</span>
              </button>
            );
          })}
        </nav>
